...
(function() {
  var CallApi, LogOff, LogOn, SetURL, fogbugzURL, https, xml2js;
  ...
  LogOff = function(token, callback) {
    var ListProjects;
    CallApi('cmd=logoff', token, function(err, result) {
      if (err) {
    ...
    return ListProjects = function(options, token, callback) {
      var command;
  ...
  CallApi = function(commandText, token, callback) {
    ...
    xml = '';
    https.get(fogbugzURL + commandText + GetToken(token), function(response) {
    ...
    return GetToken = function(nullableToken) {
      if (nullableToken) {
